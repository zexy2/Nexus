...
export async function searchWeb(
  ...
  if (!apiKey) {
    console.warn("[Tavily] TAVILY_API_KEY not set, using mock results");
 ...
 */
function getMockSearchResults(query: string): TavilySearchResponse {
  return {
    query,
    results: [
      {
        ...
        content: `This is a mock search result for "${query}". In production, this would contain actual web search results from Tavily API.`,
        score: 0.95,
        publishedDate: new Date().toISOString(),
      },
      {
        title: `Related: ${query} overview`,
        url: "https://example.com/result2",
        content: `Overview and background information about ${query}. This mock data simulates Tavily's AI-optimized search results.`,
        score: 0.87,
        publishedDate: new Date().toISOString(),
      },
      {
        ...
        content: `Best practices and recommendations for ${query}. Mock content for development purposes.`,
        score: 0.82,
        publishedDate: new Date().toISOString(),
      },
    ],
    answer: `Based on the search results, here's what we found about "${query}": This is a mock answer that would normally be generated by Tavily's AI. Set TAVILY_API_KEY for real results.`,
    followUpQuestions: [
      `What are the main benefits of ${query}?`,
